  ...
  private async _updatePasskey(params: PasskeyUpdateParams): Promise<AuthPasskeyUpdateResponse> {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        // After successful registration, fetch the updated user and update the session
        const { data: userData, error: userError } = await this.getUser()
        if (userError) {
          // If fetching the user fails, it's still a successful registration, but the client-side session might be stale.
          // Log the error but don't block the registration response.
          this._debug('#_verifyPasskeyRegistration()', 'Error fetching updated user after passkey registration:', userError)
        } else if (userData.user) {
          const currentSession = await getItemAsync(this.storage, this.storageKey) as Session | null;
          if (currentSession) {
            currentSession.user = userData.user;
            await this._saveSession(currentSession);
            await this._notifyAllSubscribers('USER_UPDATED', currentSession);
          }
        }
        return this._returnResult({ data, error: null }) // Return the passkey metadata as per the type definition
      })
    } catch (error) {
      if (isAuthError(error)) {
        return this._returnResult({ data: null, error })
